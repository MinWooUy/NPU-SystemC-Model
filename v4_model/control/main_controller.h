    ...
    private:
        ...
        void ctrl_process() {
            ...
            switch (state) {
                ...
                case START_COMP:
                    comp_cycles++;
                    // Turn off count enable after incntlim reads from memory
                    if (comp_cycles >= incntlim) {
                        o_act_cnt_en.write(false);
                        o_wei_cnt_en.write(false);
                    }
                    // Run computation for loop tile limit plus pipeline propagation latency
                    if (comp_cycles >= incntlim + (uint32_t)PE_LAT) {
                        state = ARRAY_CSWITCH;
                    }
                    break;
            ...
            if (state != old_state) {
                std::cout << "[FSM] " << this->name() << " State changed from " << old_state 
                          << " to " << state << " at " << sc_time_stamp() << std::endl;
            }
        }
    };
