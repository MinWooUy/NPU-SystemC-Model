...
//
// SystemC Testbench for INT8 Spec-Compliant Evaluation of SAURIA NPU Core (model v4)
//
...
class TestbenchSpecEvaluation : public sc_module {
public:
  // Clock & Reset Ports
  ...
  SC_CTOR(TestbenchSpecEvaluation) {
...
private:
  ...
  void test_process() {
    std::cout << "=============================================" << std::endl;
    std::cout << "    SAURIA INT8 Spec Compliance Evaluation TB " << std::endl;
    std::cout << "=============================================" << std::endl;

    std::cout << "[TB] Loading configuration parameters..." << std::endl;
    Config cfg = load_config("tb_data_spec/spec_config.txt");
    std::cout << "[TB] Loaded Parameters:\n"
              << "  * K: " << cfg.K << "\n"
              << "  * zero_gating_threshold: " << cfg.threshold << "\n"
              << "  * Precision Model: INT8 (asymmetric act, symmetric weight)\n"
              << "  * Accumulator: INT32" << std::endl;

    std::cout << "[TB] Loading input activations (Mat A - INT8)..." << std::endl;
    auto mat_A = load_matrix("tb_data_spec/spec_A.txt", Y_DIM, cfg.K);

    std::cout << "[TB] Loading weights (Mat B - INT8 pre-skewed)..." << std::endl;
    auto mat_B = load_matrix("tb_data_spec/spec_B.txt", cfg.K + X_DIM, X_DIM);

    std::cout << "[TB] Loading golden outputs (Mat C - INT32)..." << std::endl;
    auto mat_C = load_matrix("tb_data_spec/spec_C.txt", Y_DIM, X_DIM);
    ...
    const float epsilon = 0.5f; // INT bit-accurate epsilon
    ...
    float gflops = flops / (cycles * 2.0f); // cycle is 2ns in TB simulation

    std::cout << "=============================================" << std::endl;
    std::cout << "               Evaluation Summary            " << std::endl;
    std::cout << "=============================================" << std::endl;
    std::cout << "  * Simulation Cycles: " << cycles << " (clock period = 2ns)" << std::endl;
...
int sc_main(int argc, char *argv[]) {
  ...
  // Configure PeConfig for standard INT8 exact execution (no zero-gating here)
  PeConfig pe_cfg;
  ...
  pe_cfg.zero_gating_mult = false; // Disabled to verify exact INT8 dot product

  // Template instantiate with int8_t, int8_t, int32_t to match the exact specifications
  NpuTop<32, 32, int8_t, int8_t, int32_t, 1024, 1024, 2048, 16, 64, 1> npu(
      "NpuTop_inst", pe_cfg);
  TestbenchSpecEvaluation tb("TestbenchSpecEvaluation_inst");
