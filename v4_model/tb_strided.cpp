...
//
// Standalone SystemC Testbench for SAURIA NPU Core (Strided/Tiled Verification)
//
...
private:
    ...
    void test_process() {
        std::cout << "=============================================" << std::endl;
        std::cout << "  SAURIA Strided Standalone Core Verification " << std::endl;
        std::cout << "=============================================" << std::endl;

        std::cout << "[TB] Loading configuration parameters..." << std::endl;
        Config cfg = load_config("tb_data_strided/stand_config.txt");
        std::cout << "[TB] Loaded Parameters:\n"
                  ...
                  << "  * rows_active_mask: 0x" << std::hex << cfg.rows_active << std::dec << std::endl;

        std::cout << "[TB] Loading input activations (Mat A)..." << std::endl;
        auto mat_A = load_matrix("tb_data_strided/stand_A.txt", Y_DIM, cfg.K * cfg.act_incntstep);

        std::cout << "[TB] Loading weights (Mat B)..." << std::endl;
        int sramb_size = (cfg.K + X_DIM) * cfg.wei_incntstep;
        auto mat_B = load_matrix("tb_data_strided/stand_B.txt", sramb_size, X_DIM);

        std::cout << "[TB] Loading golden outputs (Mat C)..." << std::endl;
        auto mat_C = load_matrix("tb_data_strided/stand_C.txt", Y_DIM, X_DIM);
        ...
        // cxlim
        host_data_t cxlim_data;
        cxlim_data[0] = (float)cfg.cxlim;
        write_host_mem(CFG_REGS_OFFSET | (CFG_OUT_OFFSET + 0x00), cxlim_data, full_mask);

        // cxstep
        host_data_t cxstep_data;
        cxstep_data[0] = (float)cfg.cxstep;
        write_host_mem(CFG_REGS_OFFSET | (CFG_OUT_OFFSET + 0x04), cxstep_data, full_mask);

        // cklim
        host_data_t cklim_data;
        cklim_data[0] = (float)cfg.cklim;
        write_host_mem(CFG_REGS_OFFSET | (CFG_OUT_OFFSET + 0x08), cklim_data, full_mask);

        // ckstep
        host_data_t ckstep_data;
        ckstep_data[0] = (float)cfg.ckstep;
        write_host_mem(CFG_REGS_OFFSET | (CFG_OUT_OFFSET + 0x0C), ckstep_data, full_mask);

        // til_cylim
        host_data_t til_cylim_data;
        til_cylim_data[0] = (float)cfg.til_cylim;
        write_host_mem(CFG_REGS_OFFSET | (CFG_OUT_OFFSET + 0x10), til_cylim_data, full_mask);

        // til_cystep
        host_data_t til_cystep_data;
        til_cystep_data[0] = (float)cfg.til_cystep;
        write_host_mem(CFG_REGS_OFFSET | (CFG_OUT_OFFSET + 0x14), til_cystep_data, full_mask);

        // til_cklim
        host_data_t til_cklim_data;
        til_cklim_data[0] = (float)cfg.til_cklim;
        write_host_mem(CFG_REGS_OFFSET | (CFG_OUT_OFFSET + 0x18), til_cklim_data, full_mask);

        // til_ckstep
        host_data_t til_ckstep_data;
        til_ckstep_data[0] = (float)cfg.til_ckstep;
        write_host_mem(CFG_REGS_OFFSET | (CFG_OUT_OFFSET + 0x1C), til_ckstep_data, full_mask);

        // ncontexts
        host_data_t ncontexts_data;
        ncontexts_data[0] = (float)cfg.ncontexts;
        write_host_mem(CFG_REGS_OFFSET | (CFG_CON_OFFSET + 0x0C), ncontexts_data, full_mask);

        // preload_en
        host_data_t preload_en_data;
        preload_en_data[0] = cfg.preload_en ? 1.0f : 0.0f;
        write_host_mem(CFG_REGS_OFFSET | (CFG_CON_OFFSET + 0x10), preload_en_data, full_mask);

        // Write SRAM A
        std::cout << "[TB] Programming SRAM A (Activations) via Host AXI..." << std::endl;
        ...
        // Write SRAM B
        std::cout << "[TB] Programming SRAM B (Weights) via Host AXI..." << std::endl;
        ...
        // Start execution
        std::cout << "[TB] @ " << sc_time_stamp() << " Asserting Core Start..." << std::endl;
        wait();
        o_start.write(true);
        wait();
        o_start.write(false);
        ...
        if (completed) {
            std::cout << "[TB] @ " << sc_time_stamp() << " Core Completed Execution Successfully!" << std::endl;
        } else {
        ...
        std::cout << "[TB] Validating outputs against golden reference..." << std::endl;
                ...
                if (std::abs(read_C[y][x] - mat_C[y][x]) > epsilon) {
                    ...
                    if (mismatch_count <= 10) {
                        std::cerr << "[TB] Mismatch at (" << y << ", " << x << "): got " 
                                  << read_C[y][x] << ", expected " << mat_C[y][x] << std::endl;
                    }
                }
            }
        }
        ...
        std::cout << "=============================================" << std::endl;
        std::cout << "          Strided Verification Summary       " << std::endl;
        std::cout << "=============================================" << std::endl;
        std::cout << "  * Simulation Cycles: " << cycles << " (clock period = 10ns)" << std::endl;
        std::cout << "  * Effective Performance: " << std::fixed << std::setprecision(4) << gflops << " GFLOPs" << std::endl;
        if (test_passed) {
            std::cout << "  * STATUS: SUCCESS! All active outputs match golden reference." << std::endl;
        } else {
            std::cout << "  * STATUS: FAILED! Mismatch count: " << mismatch_count << std::endl;
        }
        std::cout << "=============================================" << std::endl;
